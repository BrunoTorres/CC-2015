...
public class InteracaoServidor extends Thread {
    ...
    public void run() {
        ...
        try {
            System.out.println("Entrou no interacaoServidor");
            ...
            int op = input.getCampo(0).getId()-128;
            System.out.println("op: "+op);

            switch (op) {
                case AtendimentoServidor.REGISTASV:
                    System.out.println("Registo de servidor novo e mandar para outros");
                    registaServidor(input);
                    break;

                case AtendimentoServidor.REGISTASVSEMRESPOSTA:                    
                    System.out.println("Registo de servidor novo sem resposta");
                    adicionaSVLocal(input);
                    break;

                case AtendimentoServidor.LISTADESVS:                    
                    System.out.println("ListaDESVS");
                    adicionaSVLocal();
                    break;
                ...
                case AtendimentoServidor.REQUESTDESAFIO:
                    System.out.println("REQUESTDESAFIO");
    ...
    private void registaServidor(PDU p) throws IOException, ClassNotFoundException { // sv principal regista novo sv, devolve-lhe lista dos que conhece e envia aos que conhece
        // o novo sv
        System.out.println("Vou registar um servidor");
        String ip = new String(p.getCampo(1).getValor());
        ...
        for (InetAddress i : this.bd.getServidores().keySet()) {            
            System.out.println("Vou mandar para o servidor: "+this.bd.getServidores().get(i) );
